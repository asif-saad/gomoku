...
''' The MakeCEngine class represents a simple game engine for playing MakeC, an AI-based game. 
 It provides functionality for making moves, evaluating game states, and 
selecting the best move for the AI player.'''


class MakeCEngine:
    def __init__(self):
        self.board = np.full((15, 15), -1, dtype=np.dtype(
            int))  # board to store moves. If user moves first: -1 - empty, 0 - user, 1 - AI
        self.positive_directions = [(0, 1), (1, 1), (1, 0), (
            1, -1)]  # direction vectors, that should be considered in evaluated multiplied by 1 and -1
        self.ai_depth = 2  # number of moves to forecast during AI move

    def getChildren(self,
                    only_closest_moves=False):  # returns coords on board that should be considered as possible moves. Dummy for now.
        if not only_closest_moves:  # If only_closest_moves is False, we find all positions on the board where the value is -1 i.e. empty cell
            positions = np.where(self.board < 0)
            positions = list(zip(positions[0], positions[1]))
        else:  # If the only_closest_moves parameter is set to True, it returns the closest neighbor positions of the original position
            positions = np.where(self.board >= 0)
            positions = list(zip(positions[0], positions[1]))
            buf_positions = []
            for position in positions:
                for direction in self.positive_directions:
                    ...
                    if 0 <= new_x <= 14 and 0 <= new_y <= 14 and self.board[new_x][new_y] == -1:
    ...
    def evaluate(self, last_moves):
        """ Very simple static evaluation, only last move is taken into consideration.
             Counts number of line segments that this move appears in and returns score for the move"""
        evaluation = 0
        ...
        for position in last_moves:
            ...
            for direction in self.positive_directions:
                ...
                while can_move:
                    ...
                    if not (0 <= cur_position[0] <= 14 and 0 <= cur_position[1] <= 14):
    ...
    def minimax(self, last_moves, d, min_val, max_val, only_closest=True,
                children=None):
        """" recursive minimax with alpha-beta pruning that performs evaluation in every node, 
        not only in leafs (thus evaluation should be very cheap)"""""
